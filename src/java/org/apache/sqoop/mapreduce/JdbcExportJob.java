...
import org.apache.sqoop.util.Base64UtilsEN;
import org.apache.sqoop.util.HttpClientConsumer;
import org.apache.sqoop.util.RSAUtilsEN;
...
public class JdbcExportJob extends ExportJobBase {
  ...
  protected void configureOutputFormat(Job job, String tableName,
    ...
    if(!options.getConnectString().contains("es@")){
    ...
    if(options.getConnectString().contains("es@")) {
      ...
      String[] split = options.getConnectString().split("@");
      String esips = split[1];
      String esclustername = split[2];
      String indexAndTypeName = split[3];
      configuration.set("es.nodes", esips);
      String url = esips;
      if(esips.contains(",")){
        url = esips.split(",")[0];
      }

      configuration.set("es.resource", indexAndTypeName);
